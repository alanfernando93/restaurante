...
class CakeResponseTest extends CakeTestCase {
	...
	public function testHeader() {
		...
		$response->header ( 'Access-Control-Allow-Origin', array (
				'domain1',
				'domain2' 
		) );
		$headers += array (
	...
	public function testSend() {
		...
		$response->expects ( $this->at ( 4 ) )->method ( '_sendHeader' )->with ( 'Access-Control-Allow-Origin', 'domain1' );
		$response->expects ( $this->at ( 5 ) )->method ( '_sendHeader' )->with ( 'Access-Control-Allow-Origin', 'domain2' );
		$response->expects ( $this->at ( 6 ) )->method ( '_sendHeader' )->with ( 'Content-Length', 17 );
		$response->expects ( $this->at ( 7 ) )->method ( '_sendHeader' )->with ( 'Content-Type', 'text/html; charset=UTF-8' );
		$response->send ();
	}
	...
	public function testFileWithUnknownFileTypeGeneric() {
		...
		$response->expects ( $this->at ( 3 ) )->method ( 'header' )->with ( 'Content-Transfer-Encoding', 'binary' );
	...
	public function testFileWithUnknownFileTypeOpera() {
		...
		$response->expects ( $this->at ( 1 ) )->method ( 'type' )->with ( 'application/octet-stream' )->will ( $this->returnValue ( false ) );
		
		$response->expects ( $this->once () )->method ( 'download' )->with ( 'no_section.ini' );
		
		$response->expects ( $this->at ( 3 ) )->method ( 'header' )->with ( 'Accept-Ranges', 'bytes' );
		
		$response->expects ( $this->at ( 4 ) )->method ( 'header' )->with ( 'Content-Transfer-Encoding', 'binary' );
		
		$response->expects ( $this->at ( 5 ) )->method ( 'header' )->with ( 'Content-Length', 35 );
	...
	public function testFileRangeOffsets($range, $length, $offsetResponse) {
		...
		$response->expects ( $this->at ( 4 ) )->method ( 'header' )->with ( array (
