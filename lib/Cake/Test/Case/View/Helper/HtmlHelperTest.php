...
class HtmlHelperTest extends CakeTestCase {
	...
	public function testImageQueryString() {
		...
		$this->assertTags ( $result, array (
				'img' => array (
						'src' => '/images/display/test?one=two&amp;three=four',
						'alt' => '' 
				) 
		) );
	}
	...
	public function testImagePathPrefix() {
		$result = $this->Html->image ( 'test.gif', array (
				'pathPrefix' => '/my/custom/path/' 
		) );
		$this->assertTags ( $result, array (
				'img' => array (
						'src' => '/my/custom/path/test.gif',
						'alt' => '' 
				) 
		) );
		
		$result = $this->Html->image ( 'test.gif', array (
				'pathPrefix' => 'http://cakephp.org/assets/img/' 
		) );
		$this->assertTags ( $result, array (
				'img' => array (
		...
		$result = $this->Html->image ( 'test.gif', array (
				'pathPrefix' => '//cakephp.org/assets/img/' 
		) );
		$this->assertTags ( $result, array (
				'img' => array (
		...
		$this->assertTags ( $result, array (
				'img' => array (
						'src' => '//cdn.cakephp.org/img/test.gif',
						'alt' => '' 
				) 
		) );
	...
	public function testCssLink() {
		...
		$result = $this->Html->css ( 'TestPlugin.style', array (
				'plugin' => false 
		) );
		...
		$result = $this->Html->css ( 'cake.generic', array (
				'pathPrefix' => '/my/custom/path/' 
		) );
		...
		$result = $this->Html->css ( 'cake.generic', array (
				'pathPrefix' => 'http://cakephp.org/assets/css/' 
		) );
		...
		$result = $this->Html->css ( 'css_in_head', array (
				'inline' => false 
		) );
		$this->assertNull ( $result );
		
		$result = $this->Html->css ( 'more_css_in_head', array (
				'inline' => false 
		) );
		$this->assertNull ( $result );
		
		$result = $this->Html->css ( 'screen', array (
				'rel' => 'import' 
		) );
		$expected = array (
				'style' => array (
	...
	public function testScript() {
		...
		$result = $this->Html->script ( 'foo2', array (
				'pathPrefix' => '/my/custom/path/' 
		) );
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => '/my/custom/path/foo2.js' 
				) 
		);
		$this->assertTags ( $result, $expected );
		
		$result = $this->Html->script ( 'foo3', array (
				'pathPrefix' => 'http://cakephp.org/assets/js/' 
		) );
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => 'http://cakephp.org/assets/js/foo3.js' 
				) 
		);
		$this->assertTags ( $result, $expected );
		...
		$expected = array (
				'script' => array (
						'type' => 'text/javascript',
						'src' => '//cdn.cakephp.org/js/foo4.js' 
				) 
		);
		$this->assertTags ( $result, $expected );
	...
	public function testMedia() {
		...
		$expected = array (
				'video' => array (
						'poster' => Configure::read ( 'App.imageBaseUrl' ) . 'poster.jpg' 
				),
				array (
						'source' => array (
