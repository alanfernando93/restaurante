...
class Model extends Object implements CakeEventListener {
	 ...
	 *        	Set this ID for this model on startup,
	...
	public function saveAssociated($data = null, $options = array()) {
		...
		foreach ( $data as $association => $values ) {
			...
			if ($validates) {
				if ($options ['deep']) {
					$saved = $Model->saveAssociated ( $values, array_merge ( $options, array (
							'atomic' => false 
					) ) );
				} else {
					$saved = $Model->save ( $values, array_merge ( $options, array (
							'atomic' => false 
					) ) );
				}
		...
		foreach ( $data as $association => $values ) {
			...
			switch ($type) {
				...
				case 'hasMany' :
					...
					$options = $Model->_addToWhiteList ( $key, $options );
					$_return = $Model->saveMany ( $values, array_merge ( $options, array (
							'atomic' => false 
					) ) );
	...
	/**
	 * Handles the before/after filter logic for find('all') operations.
